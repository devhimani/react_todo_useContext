...
const Todo = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setTodos([...todos, { id: Date.now(), name: input, isCheckCompleted: false }]);
    setInput("");
  ...
  const handleUpdateTodo = (todoId) => {
    setTodos(todos.map((todo) => (todoId === todo.id ? { ...todo, name: newInput } : todo)));
    setEditIndex(null);
  };

  const handleDone = (todoId) => {
    setTodos(todos.map((todo) => (todoId === todo.id ? { ...todo, isCheckCompleted: !todo.isCheckCompleted } : todo)));
  };
              ...
              .map((todo) => (
                <li key={todo.id} className="todo-item">
                  {editIndex === todo.id ? (
                    ...
                    <div className={`todo-checkbox ${todo.isCheckCompleted ? "done" : ""}`}>
                      <input type="checkbox" checked={todo.isCheckCompleted} onChange={() => handleDone(todo.id)} />
                      <p>{todo.name}</p>
                    </div>
                  )}
                  <div>
